...
import type { AvailabilityDotTone, DoctorAppointmentCalendarMonth } from '../../types/doctorAppointments.types';

export interface DoctorCalendarGridProps {
...
const DOT_TONE_CLASS: Record<AvailabilityDotTone, string> = {
  blue: 'bg-primary-500',
  green: 'bg-green-500',
        ...
        <View className="mt-7 flex-row">
          {weekdayLabels.map((label) => (
            <View key={label} className="flex-1 items-center px-[10px] py-[3px]">
              <Text className="text-b4 font-medium font-sans text-grey-900">{label}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <View className="rounded-bl-xl rounded-br-xl bg-white">
        <View className="px-3 pb-5 pt-1">
          {month.weeks.map((week, weekIndex) => (
            <View key={`doctor-calendar-week-${weekIndex}`} className="flex-row">
              {week.map((day, dayIndex) => {
                if (!day) {
                  return (
                    <View
                      key={`doctor-calendar-empty-${weekIndex}-${dayIndex}`}
                ...
                return (
                  ...
                  >
                    <View className="items-center gap-2">
                      <View
                        className={[
                          'min-w-[28px] items-center justify-center rounded-xs px-2',
