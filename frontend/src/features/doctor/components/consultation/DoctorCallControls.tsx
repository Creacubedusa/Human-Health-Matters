...
export interface DoctorCallControlsProps {
  muted: boolean;
  videoOn: boolean;
  onPressAudio: () => void;
  onPressVideo: () => void;
  onPressPatientChat: () => void;
  onPressSoapAI: () => void;
  onPressEndCall: () => void;
}
...
export function DoctorCallControls({
  muted,
  videoOn,
  onPressAudio,
  onPressVideo,
  ...
  return (
    <View className="flex-row items-center justify-center gap-4">
      ...
      <Pressable
        ...
        onPress={onPressPatientChat}
        accessibilityRole="button"
        accessibilityLabel="Open patient chat"
      >
        <Ionicons name="chatbubble-outline" size={20} color={primitiveColors.white} />
      </Pressable>

      <Pressable
        className={`${BTN} bg-primary-100`}
        onPress={onPressSoapAI}
        accessibilityRole="button"
        accessibilityLabel="Open SOAP AI assistant"
      >
