  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        if (mounted) setAvatarUri(res.profile?.avatarUri ?? null);
      } catch {
      ...
      <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="pt-6 pb-4 flex-row items-center justify-between">
          <View className="flex-row items-center gap-3">
            <View className="w-10 h-10 rounded-full bg-primary-100 items-center justify-center overflow-hidden">
              {avatarUri ? (
                <Image source={{ uri: avatarUri }} style={{ width: 40, height: 40 }} contentFit="cover" />
              ) : (
                <Text className="text-b2 font-semibold font-sans text-primary-600">
                  {dashboard.doctorName?.trim()?.[0]?.toUpperCase() ?? 'D'}
                </Text>
              )}
            </View>
            <View>
              <Text className="text-h5 text-text-primary">{greeting}</Text>
              <Text className="text-b3 text-text-secondary mt-0.5">{t('doctorHome.subtitle')}</Text>
            </View>
          </View>
          ...
          <Pressable
            ...
            accessibilityRole="button"
          >
            <Text className="text-b2 text-primary-500">{t('doctorHome.viewProfile')}</Text>
          </Pressable>
        </View>
