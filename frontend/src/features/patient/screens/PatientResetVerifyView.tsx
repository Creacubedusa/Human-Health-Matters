...
import { usePatientResetVerify } from '../hooks/usePatientResetVerify';
import { useAuthStore } from '@shared/store/auth.store';

const CODE_LENGTH = 6;

export interface PatientResetVerifyViewProps {
  onSuccess: () => void;
}
...
export function PatientResetVerifyView({ onSuccess }: PatientResetVerifyViewProps) {
      ...
      >
        ...
        >
          ...
          <View className="items-center gap-14 w-full">
            <View className="gap-2 w-full">
              <Text className="text-h4 font-semibold font-sans text-grey-900 text-center">
                {t('patientResetVerify.title')}
              </Text>
              <View className="flex-row flex-wrap justify-center">
                <Text className="text-b1 font-sans text-grey-900">
                  {t('patientResetVerify.subtitlePrefix')}{' '}
                </Text>
          ...
          <View className="items-center gap-6 w-full">
            ...
            <Button
              label={t('patientResetVerify.verify')}
            ...
            >
              {({ pressed }) => (
                <View className="flex-row items-center gap-2">
                  ...
                  >
                    {t('patientResetVerify.resendCode')}
                  </Text>
                </View>
              )}
            </Pressable>
