...
export function inputStyle({ theme, focused = false, hasError = false, }) {
    return {
...
const TextInputWithSwitcher = forwardRef((props, ref) => {
    const { isMobile } = useLayout();
    const context = useBottomSheetInternal(true);
    const isBottomSheet = context !== null;
    return isMobile && IsNative && isBottomSheet ? (
    // @ts-ignore
    _jsx(BottomSheetTextInput, { ref: ref, ...props })) : (_jsx(TextInput, { ref: ref, ...props }));
});
TextInputWithSwitcher.displayName = 'TextInputWithSwitcher';
export const Input = styled(TextInputWithSwitcher)(({ theme, focused, hasError }) => ({
