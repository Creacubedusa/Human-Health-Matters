...
export const SelectModal = ({ data = [], onChange, onModalOpen, onModalClose, keyExtractor = (item) => String(item.key || item.id || item), labelExtractor = (item) => String(item.label || item.name || item), closeOnChange = true, initValue = '', listType = 'SCROLLVIEW', animationType = 'slide', style, selectStyle: selectStyleProp, selectTextStyle: selectTextStyleProp, optionStyle: optionStyleProp, optionTextStyle: optionTextStyleProp, optionContainerStyle, sectionStyle: sectionStyleProp, childrenContainerStyle, touchableStyle, touchableActiveOpacity = 0.2, sectionTextStyle: sectionTextStyleProp, cancelContainerStyle, cancelStyle: cancelStyleProp, cancelTextStyle: cancelTextStyleProp, overlayStyle: overlayStyleProp, initValueTextStyle: initValueTextStyleProp, cancelText = 'Cancel', disabled = false, supportedOrientations = ['portrait'], keyboardShouldPersistTaps = 'always', backdropPressToClose = true, openButtonContainerAccessible = true, listItemAccessible = true, cancelButtonAccessible = true, scrollViewAccessible = true, scrollViewAccessibilityLabel, cancelButtonAccessibilityLabel, passThruProps, selectTextPassThruProps, optionTextPassThruProps, cancelTextPassThruProps, scrollViewPassThruProps, modalOpenerHitSlop, customSelector, children, header, optionsTestIDPrefix = 'select-modal', onEndReached, multiselect = false, selectedSeparator = ', ', maxSelectedItems, selectedItemIndicatorStyle, selectedItemIndicatorColor = HIGHLIGHT_COLOR, doneText = 'Done', onDone, }) => {
    ...
    const renderOption = useCallback((option, isLastItem, isFirstItem) => {
        const optionLabel = labelExtractor(option);
        const optionKey = keyExtractor(option);
        const isSelected = selectedItemsMap[optionKey];
        const content = (_jsxs(_Fragment, { children: [_jsx(Text, { style: [styles.optionTextStyle, optionTextStyleProp], ...optionTextPassThruProps, children: optionLabel }), _jsx(View, { style: [styles.selectedItemIndicator, selectedItemIndicatorStyle], children: isSelected && (_jsx(Text, { style: [styles.checkmark, { color: selectedItemIndicatorColor }], children: "\u2713" })) })] }));
        return (_jsx(TouchableOpacity, { testID: `${optionsTestIDPrefix}-${optionLabel}`, onPress: () => handleChange(option), activeOpacity: touchableActiveOpacity, accessible: listItemAccessible, accessibilityLabel: option.accessibilityLabel, importantForAccessibility: isFirstItem ? 'yes' : 'no', ...passThruProps, children: _jsx(View, { style: [styles.optionStyle, optionStyleProp, isLastItem && { borderBottomWidth: 0 }], children: _jsx(View, { style: styles.optionRow, children: content }) }) }, optionKey));
    }, [
        keyExtractor,
        labelExtractor,
        handleChange,
    ...
    const renderSection = useCallback((section) => {
        ...
        return (_jsx(View, { style: [styles.sectionStyle, sectionStyleProp], children: _jsx(Text, { style: [styles.sectionTextStyle, sectionTextStyleProp], children: sectionLabel }) }, keyExtractor(section)));
    }, [keyExtractor, labelExtractor, sectionStyleProp, sectionTextStyleProp]);
    const renderFlatlistOption = useCallback(({ item, index }) => {
        if (item.section) {
    ...
    const renderOptionList = useCallback(() => {
        ...
        return (_jsx(OverlayComponent, { ...overlayProps, children: _jsxs(View, { style: [styles.overlayStyle, overlayStyleProp], children: [_jsxs(View, { style: [styles.optionContainer, optionContainerStyle], children: [header, listType === 'FLATLIST' ? (_jsx(FlatList, { data: data, keyboardShouldPersistTaps: keyboardShouldPersistTaps, accessible: scrollViewAccessible, accessibilityLabel: scrollViewAccessibilityLabel, keyExtractor: keyExtractor, renderItem: renderFlatlistOption, onEndReached: onEndReached })) : (_jsx(ScrollView, { keyboardShouldPersistTaps: keyboardShouldPersistTaps, accessible: scrollViewAccessible, accessibilityLabel: scrollViewAccessibilityLabel, ...scrollViewPassThruProps, children: _jsx(View, { style: optionsContainerStyle, children: data.map((item, index) => item.section
                                        ? renderSection(item)
                                        : renderOption(item, index === data.length - 1, index === 0)) }) }))] }), multiselect && (_jsx(View, { style: [styles.doneContainer], children: _jsx(TouchableOpacity, { style: styles.doneButton, onPress: handleDone, activeOpacity: touchableActiveOpacity, children: _jsx(Text, { style: styles.doneButtonText, children: doneText }) }) })), _jsx(View, { style: [styles.cancelContainer, cancelContainerStyle], children: _jsx(TouchableOpacity, { onPress: close, activeOpacity: touchableActiveOpacity, accessible: cancelButtonAccessible, accessibilityLabel: cancelButtonAccessibilityLabel, children: _jsx(View, { style: [styles.cancelStyle, cancelStyleProp], children: _jsx(Text, { style: [styles.cancelTextStyle, cancelTextStyleProp], ...cancelTextPassThruProps, children: cancelText }) }) }) })] }) }, key));
    }, [
        data,
    ...
    const renderChildren = useCallback(() => {
        ...
        return (_jsx(View, { style: [styles.selectStyle, selectStyleProp], children: _jsx(Text, { style: initSelectStyle, ...selectTextPassThruProps, children: selected }) }));
    }, [
        children,
        selected,
        initValue,
    ...
    return (_jsxs(View, { style: style, ...passThruProps, children: [_jsx(ModalPortal, { transparent: true, supportedOrientations: supportedOrientations, visible: modalVisible, onRequestClose: close, animationType: animationType, onDismiss: () => selectedItems.length > 0 && onChange?.(selectedItems), children: renderOptionList() }), customSelector || (_jsx(TouchableOpacity, { hitSlop: modalOpenerHitSlop, activeOpacity: touchableActiveOpacity, style: touchableStyle, onPress: open, onLongPress: handleLongPress, disabled: disabled, accessible: openButtonContainerAccessible, children: _jsx(View, { style: childrenContainerStyle, pointerEvents: "none", children: renderChildren() }) }))] }));
};
